	...
	async run(button) {
		...
		if (button.id === 'createTicket') {
			...
			if (!guild.me.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) {
				...
				channel.send(botPerm).then((m) => this.client.utils.deletableCheck(m, 10000));
				return;
			}
			...
			if (!guild.roles.cache.find((r) => r.name === 'Support Team') && !fetch.role) {
				...
				channel.send(nomodRole).then((m) => this.client.utils.deletableCheck(m, 10000));
				return;
			}
			...
			if (foundTicket.get({ authorid: button.clicker.user.id })) {
				try {
					...
					button.clicker.user.send(alreadyTicket);
					return;
				} catch {
					return;
				}
			...
			button.guild.channels.create(`ticket-${nickName}-${randomString}`, {
				...
				channel.send(newTicketE).then((m) => this.client.utils.deletableCheck(m, 4000));
