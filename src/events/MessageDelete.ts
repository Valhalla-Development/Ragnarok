...
export class MessageDelete {
    ...
    async onMessageDelete([message]: ArgsOf<'messageDelete'>) {
        ...
        if (logging) {
            ...
            const chn = message.guild?.channels.cache.get(logging.ChannelId) ?? await message.guild?.channels.fetch(logging.ChannelId);
            ...
            if (chn && chn.type === ChannelType.GuildText
                ...
                if (!deletionLog) {
                    const noLogE = new EmbedBuilder()
                        ...
                        .setDescription(`**◎ No Data:** A message sent by ${message.author} was deleted but no content was found.**`)
                        .setTimestamp();
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`**Message sent by ${message.author}, deleted in ${message.channel}**${message.content?.length ? `\n${message.content.substring(0, 3900)}` : ''}`)
