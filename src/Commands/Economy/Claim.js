			...
			if (balance.hourly !== null) {
				...
				const embed = new MessageEmbed()
					...
					.addField(`**${this.client.user.username} - Hourly**`,
						`**◎ Error:** Please wait another \`${Date.now() > balance.hourly ? 'Available!' : ms(balance.hourly - date.getTime(), { long: true })}\` before using this command.`);
				message.channel.send(embed).then((m) => this.client.utils.deletableCheck(m, 10000));
				return;
			}
			...
			if (balance.daily !== null) {
				...
				const embed = new MessageEmbed()
					...
					.addField(`**${this.client.user.username} - Daily**`,
						`**◎ Error:** Please wait another \`${Date.now() > balance.daily ? 'Available!' : ms(balance.daily - date.getTime(), { long: true })}\` before using this command.`);
				message.channel.send(embed).then((m) => this.client.utils.deletableCheck(m, 10000));
				return;
			}
