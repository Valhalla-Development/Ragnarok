...
export const CommandF = class extends Command {
  ...
  async run(message, args) {
    ...
    if (args[1] === '-a') {
      if (this.client.utils.checkOwner(message.author.id)) {
        ...
        const embed = new EmbedBuilder()
          ...
          .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
          .addFields({
            name: `**${this.client.user.username} - Give - Admin**`,
            value: `**◎ Success:** System gave <:coin:706659001164628008> \`${Number(args[2]).toLocaleString(
              'en'
    ...
    if (Number.isNaN(args[1]) || args.length > 2) {
      this.client.utils.messageDelete(message, 10000);
