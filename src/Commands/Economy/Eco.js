	...
	async run(message) {
		...
		const embed = new MessageEmbed()
			...
			.setDescription([
				...
				`\u3000`,
				`**◎ 2:** \`-shop sell all\` will add the ability to sell all sellable items you have`,
				`\u3000`,
				`**◎ 3:** \`-work\` *details unknown*`,
				`\u3000`,
				`**◎ 4:** \`-shop\` will include items you can buy, such as tools/weapons`,
				...
				`\u3000`,
				`**◎ 5:** \`-fish\` will grant the ability to fish items at a lower percentage than fish of course, you will also be required to buy bait, the higher quality bait will grant higher percentages`,
				`\u200b like \`-farm\`, you will able to level up your fishing level, granting higher quality fish`,
				`\u3000`,
				`**◎ 6:** \`-craft\` you will be able to craft temporary boosts, for example the rate at which you earn money, increased percentages etc, some will also decay and become unusable`,
				`\u3000`,
				`**◎ 7:** \`-bankrob\` the ability to rob someones bank, however several players (not decided on number yet) must join the heist`,
				`\u3000`,
				`**◎ 8:** \`-lottery\` a lottery that runs every hour, minimum 4 players, winner takes all`,
				`\u3000`,
				`**◎ 9:** A command that will create a role and give it to the richest person in the server, can be enabled or disabled by server managers`,
				`\u3000`
			]);
