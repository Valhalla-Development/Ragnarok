...
export const CommandF = class extends Command {
  ...
  async run(message, args) {
    ...
    Number.isNaN(args[0]) && args[0] === 'all' ? (coinFlip = balance.bank) : (coinFlip = args[0]);

    if (Number.isNaN(coinFlip)) {
      this.client.utils.messageDelete(message, 10000);
    ...
    if (Number(coinFlip) > balance.bank) {
      ...
      const wrongUsage = new EmbedBuilder()
        ...
        .addFields({
          name: `**${this.client.user.username} - Coin Flip**`,
          value: `**◎ Error:** You do not have enough to bet <:coin:706659001164628008> \`${Number(coinFlip).toLocaleString(
            'en'
    ...
    const initial = new EmbedBuilder()
      ...
      .addFields({
        name: `**${this.client.user.username} - Coin Flip**`,
        value: `**◎** ${message.author} bet <:coin:706659001164628008> \`${Number(coinFlip).toLocaleString(
          'en'
