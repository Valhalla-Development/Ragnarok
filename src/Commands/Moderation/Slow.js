  ...
  async run(message, args) {
    ...
    if (!guildCheck) {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields({ name: `**${this.client.user.username} - Slow**`, value: '**◎ Error:** The specified channel does not exist in this guild!' });
    ...
    if (args[0] === 'off' || args[1] === 'off') {
      ...
      const embed = new EmbedBuilder()
        .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
        .addFields({ name: `**${this.client.user.username} - Slow**`, value: `**◎ Success:** <#${channel.id}> is no longer in slowmode.` });
