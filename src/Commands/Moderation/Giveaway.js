...
module.exports = class extends Command {
	...
	async run(message, args) {
		if (!message.member.hasPermission('MANAGE_MESSAGES') && !message.member.roles.cache.some((r) => r.name === 'Giveaways')) {
			...
			message.channel.send(invalidPerm).then((m) => this.client.utils.messageDelete(m, 15000));
			return;
		}
		...
		if (!args[0]) {
			message.channel.send(usageE).then((m) => this.client.utils.messageDelete(m, 15000));
			return;
		}
		...
		if (args[0] === 'start') {
			if (!args[1] || !args[2] || !args[3]) {
				...
				message.channel.send(incorrectStart).then((m) => this.client.utils.messageDelete(m, 15000));
				return;
			}
			if (!args[1].match('[dhm]')) {
			...
			if (ms(args[1]) > '7889400000') {
				...
				message.channel.send(valueHigh).then((m) => this.client.utils.messageDelete(m, 15000));
				return;
			}
			if (ms(args[1]) < '60000') {
				...
				message.channel.send(valueLow).then((m) => this.client.utils.messageDelete(m, 15000));
				return;
			}
			if (isNaN(ms(args[1]))) {
				...
				message.channel.send(invalidDur).then((m) => this.client.utils.messageDelete(m, 15000));
				return;
			}
			if (isNaN(args[2]) || (parseInt(args[2]) <= 0)) {
				...
				message.channel.send(invalidNum).then((m) => this.client.utils.messageDelete(m, 15000));
				return;
			}
		...
		if (args[0] === 'reroll') {
			if (!args[1]) {
				...
				message.channel.send(incorrectReroll).then((m) => this.client.utils.messageDelete(m, 15000));
				return;
			}
			...
			if (!giveaway) {
				...
				message.channel.send(noGiveaway).then((m) => this.client.utils.messageDelete(m, 15000));
				return;
			}
			this.client.giveawaysManager.reroll(giveaway.messageID)
				...
				.catch((e) => {
					if (e.startsWith(`Giveaway with message ID ${giveaway.messageID} is not ended.`)) {
						...
						message.channel.send(notEnded).then((m) => this.client.utils.messageDelete(m, 15000));
					} else {
						console.error(e);
		...
		if (args[0] === 'stop') {
			if (!args[1]) {
				...
				message.channel.send(incorrectStop).then((m) => this.client.utils.messageDelete(m, 15000));
				return;
			}
			const giveaway = this.client.giveawaysManager.giveaways.find((g) => g.messageID === args[1]);
			if (!giveaway) {
			...
			this.client.giveawaysManager.edit(giveaway.messageID, {
				...
				message.channel.send(stopped).then((m) => this.client.utils.messageDelete(m, 15000));
			}).catch((e) => {
				if (e.startsWith(`Giveaway with message ID ${giveaway.messageID} is already ended.`)) {
					...
					message.channel.send(alreadyEnded).then((m) => this.client.utils.messageDelete(m, 15000));
				} else {
					console.error(e);
					const error = new MessageEmbed()
						.setColor(this.client.utils.color(message.guild.me.displayHexColor))
