  ...
  async run(message, args) {
    ...
    if (channel) {
      ...
      if (!ch) {
        const embed = new EmbedBuilder()
          .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
          .addFields({ name: `**${this.client.user.username} - Say**`, value: '**◎ Error:** I could not find the channel you mentioned!' });
        message.channel.send({ embeds: [embed] }).then((m) => this.client.utils.deletableCheck(m, 10000));
        return;
      }
      ...
      if (!message.guild.members.me.permissionsIn(ch).has(PermissionsBitField.Flags.SendMessages)) {
        const embed = new EmbedBuilder()
          .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
          .addFields({ name: `**${this.client.user.username} - Say**`, value: `**◎ Error:** I do not have permissions to send a message in ${ch}!` });
        message.channel.send({ embeds: [embed] }).then((m) => this.client.utils.deletableCheck(m, 10000));
        return;
      }
      ...
      if (!input) {
        const embed = new EmbedBuilder()
          .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
          .addFields({ name: `**${this.client.user.username} - Say**`, value: '**◎ Error:** You need to input text!' });
        message.channel.send({ embeds: [embed] }).then((m) => this.client.utils.deletableCheck(m, 10000));
        return;
      }
