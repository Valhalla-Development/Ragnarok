  ...
  async run(message, args) {
    if (!args[0]) {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields({ name: `**${this.client.user.username} - Purge**`, value: '**◎ Error:** You need to specify the amount of messages to purge!' });
    ...
    if (Number.isNaN(args[0])) {
      ...
      const embed = new EmbedBuilder()
        .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
        .addFields({ name: `**${this.client.user.username} - Purge**`, value: '**◎ Error:** The provided argument is not a valid number.' });
      message.channel.send({ embeds: [embed] }).then((m) => this.client.utils.deletableCheck(m, 10000));
      return;
    }
    if (args[0] < 1) {
