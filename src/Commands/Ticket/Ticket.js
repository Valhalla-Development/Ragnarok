	...
	async run(message, args) {
		...
		if (args[0] === undefined) {
			...
			if (!ticketGrab['count(*)']) {
				...
				message.channel.send({ embed: noTickets }).then((m) => this.client.utils.deletableCheck(m, 10000));
				return;
			}
			...
			message.channel.send({ embed: ticketList });
			return;
			...
			if (!ticketGrab) {
				...
				message.channel.send({ embed: noTickets });
				return;
			}
			...
			message.channel.send({ embed: confirmClear }).then((m) => {
				m.awaitMessages(
					...
					message.channel.send({ embed: clearedMessage });
				});
			});
		}
	}
