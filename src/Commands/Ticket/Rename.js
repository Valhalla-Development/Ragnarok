  ...
  async run(message, args) {
    ...
    if (foundTicket) {
      ...
      if (comCooldown.has(`${message.author.id}-${getChan.id}`)) {
        ...
        const donthaveRole = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
          name: `**${this.client.user.username} - Rename**`,
      ...
      if (!comCooldown.has(`${message.author.id}-${getChan.id}`)) {
        ...
        const embed = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
          name: `**${this.client.user.username} - Rename**`,
          value: `**◎ Success:** <@${message.author.id}> renamed ticket to \`${argResult}\``
        });
        ...
        if (!logchan) return;
        const loggingembed = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
          name: `**${this.client.user.username} - Rename**`,
          value: `**◎ Success:** <@${message.author.id}> renamed ticket from \`#${getChan.name}\` to <#${getChan.id}>`
        });
      ...
      if (!comCooldown.has(`${message.author.id}-${foundTicket.chanid}`)) {
        ...
        const embed = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
          name: `**${this.client.user.username} - Rename**`,
          value: `**◎ Success:** <@${message.author.id}> renamed ticket from to \`${argResult}\``
        });
        ...
        const loggingembed = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
          name: `**${this.client.user.username} - Rename**`,
          value: `**◎ Success:** <@${message.author.id}> renamed ticket from \`#${message.channel.name}\` to <#${message.channel.id}>`
        });
        logchan.send({ embeds: [loggingembed] });
