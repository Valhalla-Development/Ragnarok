...
export const CommandF = class extends Command {
  ...
  async run(message, args) {
    ...
    if (!comCooldown.has(message.author.id)) {
      ...
      const initial = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
        name: `**${this.client.user.username} - Close**`,
          ...
          if (user) {
            if (!closeReason) {
              const dmE = new EmbedBuilder()
                ...
                .addFields(
                  { name: '<:ticketId:998229977004781618> **Ticket ID**', value: `\`${channelArgs[channelArgs.length - 1]}\``, inline: true },
                ...
                .setTimestamp();
              user
                .send(transcriptRow ? { components: [transcriptRow], embeds: [dmE] } : { embeds: [dmE] })
                .then(() => {
      ...
      collector.on('end', (_, reason) => {
        ...
        if (reason === 'cancel' || reason === 'time') {
          ...
          const limitE = new EmbedBuilder()
            ...
            .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
            .addFields({ name: `**${this.client.user.username} - Close**`, value: '**◎ Success:** Ticket close cancelled.' });
      ...
      const badChannel = new EmbedBuilder()
        .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
        .addFields({ name: `**${this.client.user.username} - Close**`, value: `**◎ Error:** Please only run \`${prefix}close\` once!` });
      message.channel.send({ embeds: [badChannel] }).then((m) => this.client.utils.deletableCheck(m, 10000));
    }
  }
};
