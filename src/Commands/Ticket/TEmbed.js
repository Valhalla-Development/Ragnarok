  ...
  async run(message, args) {
    ...
    if (!message.member.roles.cache.has(modRole.id) && message.author.id !== message.guild.ownerID) {
      ...
      const donthaveRole = new EmbedBuilder()
        ...
        .addFields({ name: `**${this.client.user.username} - TicketEmbed**`, value: `**◎ Error:** Sorry! You do not have the **${modRole}** role.` });
    ...
    if (checkEmbedEx) {
      if (args[0] === 'clear') {
        if (checkEmbedEx.ticketembed) {
          ...
          const cleared = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
            name: `**${this.client.user.username} - TicketEmbed**`,
        ...
        const danelSucc = new EmbedBuilder()
          .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
          .addFields({ name: `**${this.client.user.username} - TicketEmbed**`, value: '**◎ Error:** I found no embed data in the database!' });
      ...
      if (checkEmbedEx.ticketembed === null) {
        ...
        try {
          ...
          await embedChannel.messages
            .fetch({ message: foundtEmbed.ticketembed })
            .then((res) => {
              if (res) {
                const alreadytick = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
                  name: `**${this.client.user.username} - TicketEmbed**`,
