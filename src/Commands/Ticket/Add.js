...
export const CommandF = class extends Command {
  ...
  async run(message, args) {
    ...
    if (foundTicket) {
      ...
      const nouser = new EmbedBuilder()
        .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
        .addFields({ name: `**${this.client.user.username} - Add**`, value: `**◎ Success:** ${rUser} has been added to the ticket!` });
      ...
      const loggingembed = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
        name: `**${this.client.user.username} - Add**`,
        value: `**◎ Success:** <@${message.author.id}> added ${rUser} to ticket <#${getChan.id}>`
      });
