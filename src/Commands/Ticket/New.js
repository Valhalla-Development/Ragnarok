...
export const CommandF = class extends Command {
  ...
  async run(message, args) {
    ...
    if (!fetch) {
      const alreadyTicket = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
    ...
    if (!message.guild.members.me.permissions.has(PermissionsBitField.Flags.ManageChannels)) {
      const botPerm = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
        name: `**${this.client.user.username} - Ticket**`,
    ...
    // "Support" role
    if (!fetch.role) {
      const nomodRole = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
        name: `**${this.client.user.username} - Ticket**`,
    ...
    if (foundTicket.get({ authorid: message.author.id })) {
      try {
        ...
        const alreadyTicket = new EmbedBuilder()
          ...
          .addFields({ name: `**${this.client.user.username} - Ticket**`, value: `**◎ Error:** It seems you already have a ticket open. | ${cha}` });
        message.reply({ embeds: [alreadyTicket], ephemeral: true });
        return;
      } catch (e) {
        console.log(e);
    ...
    message.guild.channels
      ...
      .then((c) => {
        const updateTicketChannel = db.prepare(
          `UPDATE tickets SET chanid = (@chanid) WHERE guildid = ${message.guild.id} AND ticketid = (@ticketid)`
        );
        ...
        const newTicketE = new EmbedBuilder()
          .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
          .addFields({ name: `**${this.client.user.username} - Ticket**`, value: `**◎ Success:** Your ticket has been created, <#${c.id}>.` });
        ...
        const buttonCloseReason = new ButtonBuilder().setStyle(ButtonStyle.Danger).setLabel('🔒 Close With Reason').setCustomId('closeTicketReason');
        ...
        if (id) {
          ...
          const logEmbed = new EmbedBuilder()
            ...
            .addFields(
              { name: '<:ticketId:998229977004781618> **Ticket ID**', value: `[${randomString}](${c.url})`, inline: true },
