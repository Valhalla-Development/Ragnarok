...
module.exports = class extends Command {
	...
	async run(message, args) {
		...
		if (!message.member.roles.cache.has(modRole.id) && message.author.id !== message.guild.ownerID) {
			...
			message.channel.send(donthaveRole).then((m) => this.client.utils.messageDelete(m, 15000));
			return;
		}
		...
		if (!rUser) {
			...
			message.channel.send(nouser).then((m) => this.client.utils.messageDelete(m, 15000));
			return;
		}
