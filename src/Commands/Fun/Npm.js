  ...
  async run(message, args) {
    if (!args[0]) {
      ...
      const noInput = new EmbedBuilder()
        ...
        .addFields({ name: `**${this.client.user.username} - Discord.js**`, value: '**◎ Error:** You must input a search term!' });
      message.channel.send({ embeds: [noInput] }).then((m) => this.client.utils.deletableCheck(m, 10000));
      return;
    }

    const res = await fetch(`https://registry.npmjs.org/${args.join('-')}`).then((r) => r.json());
    ...
    if (res.error) {
      ...
      const noInput = new EmbedBuilder()
        .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
        .addFields({ name: `**${this.client.user.username} - Discord.js**`, value: `**◎ Error:** I couldn't find anything for \`${args[0]}\`` });
      message.channel.send({ embeds: [noInput] }).then((m) => this.client.utils.deletableCheck(m, 10000));
      return;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({
        ...
        value: `**❯ Author:** ${version.maintainers[0].name || 'None'}
				...
				**❯ ${version.maintainers.length > 1 ? 'Maintainers' : 'Maintainer'}:** ${version.maintainers.map((usr) => usr.name).join(', ')}
