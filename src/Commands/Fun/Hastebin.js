  ...
  async run(message, args) {
    ...
    if (message.attachments.size > 1) {
      const fileCount = new EmbedBuilder()
        .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
        .addFields({ name: `**${this.client.user.username} - Hastebin**`, value: '**◎ Error:** You can only post 1 file at a time!' });
    ...
    if (args[0] === undefined) {
      const error = new EmbedBuilder()
        .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
        .addFields({ name: `**${this.client.user.username} - Hastebin**`, value: '**◎ Error:** You must input some text!' });
      message.channel.send({ embeds: [error] }).then((m) => this.client.utils.deletableCheck(m, 10000));
      return;
    }
