...
export const CommandF = class extends Command {
  ...
  async run(message, args) {
    ...
    let user;
    try {
      user = message.mentions.users.size
        ? message.mentions.members.first().user
        : args[0]
        ? message.guild.members.cache.find((usr) => usr.displayName === args.join(' ')).user
        : message.author;
    } catch {
      user = null;
    }
    ...
    if (user === null) {
      ...
      const limitE = new EmbedBuilder()
        ...
        .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
        .addFields({ name: `**${this.client.user.username} - Balance**`, value: '**◎ Error:** I could not find the specified user!' });
