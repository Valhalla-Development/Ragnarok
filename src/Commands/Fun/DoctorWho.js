  ...
  async run(message) {
    if (comCooldown.has(message.author.id)) {
      ...
      const embed = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
        name: `**${this.client.user.username} - Doctor Who**`,
    ...
    const buttonA = new ButtonBuilder().setStyle(ButtonStyle.Primary).setLabel('Next Post').setCustomId('nxtmeme');
    ...
    collector.on('collect', async (b) => {
      if (b.user.id !== message.author.id) {
        const wrongUser = new EmbedBuilder().setColor(this.client.utils.color(message.guild.members.me.displayHexColor)).addFields({
          name: `**${this.client.user.username} - Doctor Who**`,
