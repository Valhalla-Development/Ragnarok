		...
		if (!message.member.hasPermission('MANAGE_MESSAGES') && !message.member.roles.cache.some((r) => r.name === 'Giveaways')) {
			const invalidPerm = new MessageEmbed()
				...
				.addField('**Invalid Permissions**',
					`**◎ Invalid Permissions:** You need to have the manage messages permissions to reroll giveaways.`);
			...
			if (!args[1] || !args[2] || !args[3]) {
				const incorrectStart = new MessageEmbed()
					...
					.addField('**Incorrect Usage**',
						`**◎ Correct usage:** \`${prefix}giveaway start <time> <winners amount> <prize>\``);
			...
			if (!args[1].match('[dhm]')) {
				const incorrectFormat = new MessageEmbed()
					...
					.addField('**Incorrect Format**',
						`**◎ Correct format:** You did not use the correct formatting for the time! The valid options are \`d\`, \`h\`, or \`m\``);
			...
			if (ms(args[1]) > '7889400000') {
				const valueHigh = new MessageEmbed()
					...
					.addField('**Value Too High**',
						`**◎ Time Limit:** Please input a value lower than 3 months!`);
			...
			if (isNaN(ms(args[1]))) {
				const invalidDur = new MessageEmbed()
					...
					.addField('**Invalid Duration**',
						`**◎ Invalid Duration:** Please input a valid duration!`);
		...
		if (args[0] === 'reroll') {
			if (!args[1]) {
				const incorrectReroll = new MessageEmbed()
					.setColor(message.guild.me.displayHexColor || message.guild.me.displayHexColor || '36393F')
					.addField('**Incorrect Usage**',
						`**◎ Correct usage:** \`${prefix}giveaway reroll <message id>\``);
