...
const subreddits = ['dog', 'dogpics', 'puppies'];
  ...
  async run(message) {
    if (comCooldown.has(message.author.id)) {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields({ name: `**${this.client.user.username} - Dog**`, value: '**◎ Error:** Please only run this command once.' });
    ...
    const buttonA = new ButtonBuilder().setStyle(ButtonStyle.Primary).setLabel('Next Dog').setCustomId('nxtmeme');
    ...
    collector.on('collect', async (b) => {
      if (b.user.id !== message.author.id) {
        const wrongUser = new EmbedBuilder()
          .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
          .addFields({ name: `**${this.client.user.username} - Dog**`, value: '**◎ Error:** Only the command executor can select an option!' });
