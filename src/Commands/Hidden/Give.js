...
module.exports = class extends Command {
	...
	async run(message, args) {
		...
		if (!otherB) {
			...
			message.channel.send(errorE).then((m) => this.client.utils.messageDelete(m, 15000));
			return;
		}
		...
		if (balance.bank === 0) {
			...
			message.channel.send(noBal).then((m) => this.client.utils.messageDelete(m, 15000));
			return;
		}
		...
		if (args[1] === 'all') {
			if (otherB.bank + balance.bank > bankLimit) {
				...
				message.channel.send(youViolatedTheLaw).then((m) => this.client.utils.messageDelete(m, 15000));
				return;
			}
