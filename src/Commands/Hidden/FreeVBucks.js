...
export const CommandF = class extends Command {
  ...
  async run(message) {
    ...
    const buttonA = new ButtonBuilder().setStyle(ButtonStyle.Success).setLabel('Yes!').setCustomId('yes');
    ...
    const buttonANew = new ButtonBuilder().setStyle(ButtonStyle.Success).setLabel('Yes!').setCustomId('yes').setDisabled(true);

    const buttonBNew = new ButtonBuilder().setStyle(ButtonStyle.Danger).setLabel('No!').setCustomId('no').setDisabled(true);

    const rowNew = new ActionRowBuilder().addComponents(buttonANew, buttonBNew);

    const embed = new EmbedBuilder()
      .setAuthor({ name: `${message.author.tag}`, iconURL: message.author.avatarURL() })
      .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
      .addFields({
        name: `**${this.client.user.username} - Free V-Bucks**`,
    ...
    const embedNew = new EmbedBuilder()
      .setAuthor({ name: `${message.author.tag}`, iconURL: message.author.avatarURL() })
      .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
      .addFields({ name: `**${this.client.user.username} - Free V-Bucks**`, value: `**◎ Success:** ${message.author}, Virus activated!.` });
