  ...
  async run(message, [command]) {
    ...
    if (comCooldown.has(message.author.id)) {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields({ name: `**${this.client.user.username} - Help**`, value: '**◎ Error:** Please only run this command once.' });
    ...
    if (command === 'all') {
      ...
      const categories = this.client.utils.removeDuplicates(
        this.client.commands.filter((cmd) => cmd.category !== 'Hidden').map((cmd) => cmd.category)
      );

      for (const category of categories) {
        embed.addFields({
          name: `**${this.client.utils.capitalise(category)} (${this.client.commands.filter((cmd) => cmd.category === category).size})**`,
          value: this.client.commands
            .filter((cmd) => cmd.category === category)
            .map((cmd) => `\`${this.client.utils.capitalise(cmd.name)}\``)
            .join(', ')
        });
      }
      ...
      const buttonB = new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel('Support Server').setURL('https://discord.gg/Q3ZhdRJ');
    ...
    if (command) {
      ...
      if (!cmd || cmd.category === 'Hidden') {
        ...
        const embed1 = new EmbedBuilder()
          .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
          .addFields({ name: `**${this.client.user.username} - Help**`, value: `**◎ Error:** Invalid command name: \`${command}\`` });
      ...
      const buttonA = new ButtonBuilder().setStyle(ButtonStyle.Primary).setLabel('Economy').setCustomId('eco');

      const buttonB = new ButtonBuilder().setStyle(ButtonStyle.Primary).setLabel('Fun').setCustomId('fun');

      const buttonC = new ButtonBuilder().setStyle(ButtonStyle.Primary).setLabel('Generators').setCustomId('gens');

      const buttonD = new ButtonBuilder().setStyle(ButtonStyle.Primary).setLabel('Information').setCustomId('info');

      const buttonE = new ButtonBuilder().setStyle(ButtonStyle.Primary).setLabel('Moderation').setCustomId('mod');

      const buttonG = new ButtonBuilder().setStyle(ButtonStyle.Primary).setLabel('Ticket').setCustomId('ticket');
      ...
      const supp = new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel('Support Server').setURL('https://discord.gg/Q3ZhdRJ');
      ...
      const row2 = new ActionRowBuilder().addComponents(buttonE, buttonG, inv, supp, support);
      ...
      collector.on('collect', async (b) => {
        if (b.user.id !== message.author.id) {
          const wrongUser = new EmbedBuilder()
            .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
            .addFields({ name: `**${this.client.user.username} - Help**`, value: '**◎ Error:** Only the command executor can select an option!' });
        ...
        if (b.customId === 'home') {
          ...
          const row2New = new ActionRowBuilder().addComponents(buttonE, buttonG, inv, supp, support);
        ...
        if (b.customId === 'eco') {
          ...
          const categories = this.client.utils.removeDuplicates(
            this.client.commands.filter((cmd) => cmd.category === 'Economy').map((cmd) => cmd.name)
          );
          ...
          const eco = new EmbedBuilder()
            ...
            .addFields({ name: '**Help - Economy**', value: `\`${categories.join('`, `')}\`` })
            .setAuthor({ name: `${message.guild.name} Help`, iconURL: message.guild.iconURL({ extension: 'png' }) })
            .setThumbnail(this.client.user.displayAvatarURL())
            .setFooter({
              text: `This guild's prefix is ${prefix} - Bot Version ${version}`,
        ...
        if (b.customId === 'fun') {
          ...
          const fun = new EmbedBuilder()
            ...
            .addFields({ name: '**Help - Fun**', value: `\`${categories.join('`, `')}\`` })
            .setAuthor({ name: `${message.guild.name} Help`, iconURL: message.guild.iconURL({ extension: 'png' }) })
            .setThumbnail(this.client.user.displayAvatarURL())
            .setFooter({
              text: `This guild's prefix is ${prefix} - Bot Version ${version}`,
        ...
        if (b.customId === 'gens') {
          ...
          const categories = this.client.utils.removeDuplicates(
            this.client.commands.filter((cmd) => cmd.category === 'Generators').map((cmd) => cmd.name)
          );
          ...
          const gen = new EmbedBuilder()
            ...
            .addFields({ name: '**Help - Generators**', value: `\`${categories.join('`, `')}\`` })
            .setAuthor({ name: `${message.guild.name} Help`, iconURL: message.guild.iconURL({ extension: 'png' }) })
            .setThumbnail(this.client.user.displayAvatarURL())
            .setFooter({
              text: `This guild's prefix is ${prefix} - Bot Version ${version}`,
        ...
        if (b.customId === 'info') {
          ...
          const categories = this.client.utils.removeDuplicates(
            this.client.commands.filter((cmd) => cmd.category === 'Informative').map((cmd) => cmd.name)
          );
          ...
          const info = new EmbedBuilder()
            ...
            .addFields({ name: '**Help - Informative**', value: `\`${categories.join('`, `')}\`` })
            .setAuthor({ name: `${message.guild.name} Help`, iconURL: message.guild.iconURL({ extension: 'png' }) })
            .setThumbnail(this.client.user.displayAvatarURL())
            .setFooter({
              text: `This guild's prefix is ${prefix} - Bot Version ${version}`,
        ...
        if (b.customId === 'mod') {
          ...
          const mod = new EmbedBuilder()
            ...
            .addFields({ name: '**Help - Moderation**', value: `\`${categories.join('`, `')}\`` })
            .setAuthor({ name: `${message.guild.name} Help`, iconURL: message.guild.iconURL({ extension: 'png' }) })
            .setThumbnail(this.client.user.displayAvatarURL())
            .setFooter({
              text: `This guild's prefix is ${prefix} - Bot Version ${version}`,
        ...
        if (b.customId === 'ticket') {
          ...
          const categories = this.client.utils.removeDuplicates(
            this.client.commands.filter((cmd) => cmd.category === 'Ticket').map((cmd) => cmd.name)
          );
          ...
          const tick = new EmbedBuilder()
            ...
            .addFields({ name: '**Help - Ticket**', value: `\`${categories.join('`, `')}\`` })
            .setAuthor({ name: `${message.guild.name} Help`, iconURL: message.guild.iconURL({ extension: 'png' }) })
            .setThumbnail(this.client.user.displayAvatarURL())
            .setFooter({
              text: `This guild's prefix is ${prefix} - Bot Version ${version}`,
