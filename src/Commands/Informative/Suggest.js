...
const { supportGuild, suggestChan } = configFile.default;
  ...
  async run(message, args) {
    if (!args[0]) {
      ...
      const noinEmbed = new EmbedBuilder()
        ...
        .addFields({ name: `**${this.client.user.username} - Suggest**`, value: '**◎ Error:** Please input some text!' });
    ...
    collector.on('collect', async (b) => {
      ...
      if (b.customId === 'yes') {
        ...
        this.client.guilds.cache
          .get(supportGuild)
          .channels.cache.get(suggestChan)
          .send({ embeds: [embed] });
        ...
        const loggedEmbed = new EmbedBuilder()
          .setAuthor({ name: `${message.author.tag}`, iconURL: message.author.avatarURL() })
          .setColor(this.client.utils.color(message.guild.members.me.displayHexColor))
          .addFields({ name: `**${this.client.user.username} - Suggest**`, value: '**◎ Success:** Suggestion has been successfully sent!' });
