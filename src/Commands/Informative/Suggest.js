...
module.exports = class extends Command {
	...
	async run(message, args) {
		...
		this.client.guilds.cache.get(supportGuild).channels.cache.get(suggestChan).send(embed);
		...
		message.channel.send(loggedEmbed).then((m) => this.client.utils.deletableCheck(m, 10000));
	}
