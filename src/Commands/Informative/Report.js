...
module.exports = class extends Command {
	...
	async run(message, args) {
		...
		if (!target) {
			...
			message.channel.send(noTarget).then((m) => this.client.utils.deletableCheck(m, 10000));
			return;
		}
		if (!reason) {
			this.client.utils.messageDelete(message, 10000);
			...
			message.channel.send(noReason).then((m) => this.client.utils.deletableCheck(m, 10000));
			return;
		}
		if (!reports) {
			this.client.utils.messageDelete(message, 10000);
		...
		message.channel.send(success).then((m) => this.client.utils.deletableCheck(m, 10000));
	}
