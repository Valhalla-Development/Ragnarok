...
module.exports.run = async (client, message, args, color) => {
  ...
  let foundTicket = db.prepare(`SELECT * FROM tickets WHERE guildid = ${message.guild.id} AND ticketid = (@ticketid)`).get({ticketid: args[0] || channelArgs[channelArgs.length - 1]});
  if (foundTicket) {
    ...
    getChan.send(forceclosetimer).then(timerMsg => {
      getChan.awaitMessages(resp => resp.author.id === message.author.id || foundTicket.authorid, {
        ...
        let cancelTimer = new Discord.RichEmbed()
          .setColor(`#36393F`)
