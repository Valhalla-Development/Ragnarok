...
module.exports.run = async (client, message, args, color) => {
    ...
    if (args[0] == undefined) {
        ...
        if (!ticketGrab) {
            ...
            const confirmClear = new Discord.RichEmbed()
                .setColor(0xCF40FA)
                .setDescription(`**WARNING** If you use this command while you have tickets open, you will **lose functionality** of the \`${prefix}close\` & \`${prefix}confirm\` commands for those tickets!\nIf you wish to continue with this command, reply with the message \`${prefix}confirm\`\nYou have 20 seconds to reply.`);
            message.channel.send(confirmClear).then((m) => {
                    message.channel.awaitMessages(response => response.content === prefix + 'confirm', {
                        ...
                        .then(() => {
                            ...
                            let clearedMessage = new Discord.RichEmbed()
                                .setColor(0xCF40FA)
                                .setDescription(`All tickets succesfully cleared from the database!`);
