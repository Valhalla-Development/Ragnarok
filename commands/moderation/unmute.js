...
module.exports = {
    ...
    run: async (bot, message, args, color) => {
        ...
        if ((!message.member.hasPermission("KICK_MEMBERS") && (message.author.id !== ownerID))) {
            message.channel.send(`${language.unmute.noAuthorPermission}`).then(message => message.delete({ timeout: 5000 }));
            return;
        }

        const mod = message.author;
        let user = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));
        if (!user) return message.reply(`${language.unmute.noMention}`).then(message => message.delete({ timeout: 5000 }));
