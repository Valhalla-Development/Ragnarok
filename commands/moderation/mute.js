...
module.exports = {
    ...
    run: async (bot, message, args, color) => {
        ...
        if ((!message.member.hasPermission("KICK_MEMBERS") && (message.author.id !== ownerID))) {
            message.channel.send(`${language.mute.noAuthorPermission}`).then(message => message.delete({
                timeout: 5000
            }));
            return;
        }
        ...
        let noUserMessage = language.mute.noUser;
        const noUserConvert = noUserMessage.replace(
            "${prefix}",
            prefix
        );
        if (!user) return message.reply(`${noUserConvert}`).then(message => message.delete({
            timeout: 5000
        }));
        let reason = message.content.split(' ').splice(3).join(' ');
        if (!reason) return message.channel.send(`${language.mute.noReason}`).then(message => message.delete({
            timeout: 5000
        }));
        ...
        let mutetime = args[1];
        if (!mutetime) return message.channel.send(`${language.mute.noTime}`).then(message => message.delete({
            timeout: 5000
        }));
